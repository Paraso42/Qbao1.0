...
> 占位符：{PROD_ROOT} = 服务器部署根（真实值仅存 local/ENV.md，不公开）；执行以下命令前请先替换。
>
...
1. 创建部署用户与目录：代码位于 `{PROD_ROOT}`（含 server/、app/dist、downloads/、uploads/）。
2. 上传单元文件并注册：
...
3. 有数据库变更时执行迁移：`cd {PROD_ROOT}/server && node scripts/run_migration.js`。
...
- `WorkingDirectory` 与 `server/.env` 路径固定在 `{PROD_ROOT}/server`（部署前替换占位符）；服务不读取 `EnvironmentFile`，由 `server.js` 内 dotenv 加载 `.env`。
